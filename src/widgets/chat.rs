...
pub struct Chat<'a> {
    ...
    pub generating_response: Arc<Mutex<bool>>,
    /// The response being generated by the LLM
...
impl<'a> Chat<'a> {
    ...
    pub fn start_generating(&mut self) {
        let is_generating = *self.generating_response.lock().unwrap();
        if is_generating {
            return;
        }

        let generating_response = self.generating_response.clone();
            ...
            while let Some(chunk) = stream.next().await {
                if let Ok(chunk) = chunk {
                    ...
                    if let Ok(parsed) = serde_json::from_str::<Chunk>(&text) {
                        let mut buf = response.lock().unwrap();
                        buf.push_str(&parsed.message.content);
                    }
                }
            }
            *generating_response.lock().unwrap() = false;
        });
    }
}
